...
| 撮影スポットのガイド（**下書き**） | `/spots` ＋ `/spots/<slug>`（`content/spots.json`・review 段階・noindex） | `OfficialSpotView`（**索引 `app/data/spots.json` だけを読む・v1**）。入口は地図のピン（寄せたとき・名前で絞ったとき）。見どころ・季節・アクセスの本文は出さない。「行きたい」は `SPOT-<slug>` で端末に保存（`SavedSpotKey`＝Web の `savedSpotKey.ts`） |
