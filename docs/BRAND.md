...
| マーク | 白いアパーチャ 28px（768px 以上で 32px）、文字との間 8px | `Image("BrandMark")` 28pt・`spacing: 8` |
...
- `Sources/JourneyPhoto/Assets.xcassets/BrandMark.imageset`（32 / 64 / 96px）
- 白いアパーチャ・**テンプレート画像**（`foregroundStyle` で色を決められる）
- サイトの `public/logo-aperture.png`（64px）と画素で突き合わせて、透明度の差は
  平均 0.37 / 255（縮小の計算方法の違いだけ）
