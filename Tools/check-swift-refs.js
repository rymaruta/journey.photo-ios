...
// 4.6b 地図の「見えている範囲」を知らせていないか
//
// **地図を動かすたびに知らせを出すと、画面が回り続ける。**
// 描き直す → カメラが動いたと知らされる → 知らせる → 描き直す…
// 実機では**マップのタブを押した瞬間にアプリが固まった**
// （CI の UI テストが4分待って応答を得られず落ちた・run 37）。
// 手元の模型は `@Published` を素通しするので、**ここでは絶対に出ない**。
//
// 見えている範囲は描画に要らない（押したときに読めればよい）ので、
// `@Published` を付けない。
{
    for (const file of files) {
        const source = fs.readFileSync(file, "utf8");
        if (/@Published[^\n]*\bvisibleFrame\b/.test(source)) {
            problems.push(
                `${path.relative(process.cwd(), file)}: 地図の見えている範囲を @Published にしています` +
                `（描き直しとカメラの知らせが回り続け、実機でアプリが固まります）`
            );
        }
    }
}
...
// 4.6f 実機の絵の巡回から、画面が黙って消えていないか
//
// 🔴 **run 62 で `20-写真の詳細` と `21-人のページ` が丸ごと欠けた。**
// 旅の節を書き直したとき、その後ろにあった写真詳細の節を**巻き添えで
// 消していた**。巡回は「出なければ撮らない」作りなので**赤くならず**、
// 気づけるのは**絵の枚数が減ったこと**だけ——しかも12枚と14枚の違いは
// 目で追いにくい。
//
// **撮るはずの画面を、ここに書いておく。** 減らすなら、この表からも
// 意図して消すことになる（黙っては減らない）。
{
    // タブの4枚（10 ホーム / 11 探す / 13 マップ / 14 マイページ）は
    // 繰り返しの中で名前を組み立てるので、**その1行**を見る
    const wanted = [
        "01-同意画面", "1\\(index)-", "15-マイページ（下）",
        "20-写真の詳細", "21-人のページ", "30-旅の一冊", "31-旅の足取り",
        "40-投稿の2択", "41-ストーリー作成", "60-撮影スポット", "61-撮影スポット（下）",
    ];
    // **`files` は Sources しか見ていない。** 巡回は `UITests/` に在るので
    // 決め打ちで開く（見つからなければ、それ自体を問題として挙げる）
    const walk = "UITests/JourneyPhotoUITests/ScreenshotTests.swift";
    if (!fs.existsSync(walk)) {
        problems.push(`${walk} が見つかりません（実機の絵の巡回そのものが消えています）`);
    } else {
        const source = fs.readFileSync(walk, "utf8");
        const shots = source
            .split("\n")
            .filter((line) => !line.trim().startsWith("//") && line.includes("shoot(app,"))
            .join("\n");
        for (const name of wanted) {
            if (!shots.includes(name)) {
                problems.push(
                    `${walk}: 「${name}」を撮る手が消えています` +
                    "（巡回は出なければ撮らないので赤くなりません。意図して減らすなら 4.6f の表も直してください）"
                );
            }
        }
    }
